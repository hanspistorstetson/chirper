defmodule Twitter.AuthTest do
  use Twitter.DataCase

  alias Twitter.Auth

  describe "users" do
    alias Twitter.Auth.User

    @password "password"
    @updated_password "update password"
    @valid_attrs %{email: "some email", is_active: true, password: @password}
    @update_attrs %{email: "some updated email", is_active: false, password: @updated_password}
    ...
    test "update_user/2 with valid data updates the user" do
      ...
      assert Bcrypt.verify_pass(@updated_password, user.password_hash)
    end
    ...
    test "authenticate_user/2 authenticates the user" do
      ...
      assert {:ok, authenticated_user} = Auth.authenticate_user(user.email, @password)
